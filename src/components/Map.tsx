...
import { getFillLayerConfig, getOutlineLayerConfig, getLabelLayerConfig, getOccupancyLabelLayerConfig } from '@/lib/mapLayers'
...
export default function Map({ buildings, occupancyMap, onBuildingClick }: MapProps) {
  ...
  // Add building layers
  const addLayers = useCallback((map: mapboxgl.Map, buildings: Building[], occMap?: Map<string, BlendedOccupancy>) => {
    if (buildings.length === 0) return
    if (map.getSource(BUILDINGS_SOURCE)) return

    const geojson = buildingsToFeatureCollection(buildings, occMap)
    ...
    map.addLayer(getOccupancyLabelLayerConfig())
  }, [])
  ...
  useEffect(() => {
    ...
    if (map.isStyleLoaded()) addLayers(map, buildings, occupancyMap)
    const handler = () => addLayers(map, buildings, occupancyMap)
