...
- Resolve to absolute path: `flint resolve codebase PULSE`
  - Git repositories include a live `Worktrees:` block from `git worktree list`.
- Fulfill (first time on a new machine): `flint fulfill codebase PULSE <path>`
- Manage git worktrees: `flint worktree list PULSE`, `flint worktree add PULSE <suffix>`, `flint worktree remove PULSE <branch|suffix>`
